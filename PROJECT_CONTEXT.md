...
Le 13 septembre 2026, la séparation réelle de la Bibliothèque et de Ma liste a été implémentée sur la branche `codex/watchlist-navigation`, sans migration Room. Les tables historiques conservent une seule ligne par identifiant TMDB : `fileId != null` désigne la Bibliothèque Drive et `fileId == null` désigne Ma liste. Lorsqu'une indexation retrouve un média de Ma liste sur Drive, l'upsert existant lui attribue le fichier ou dossier Drive et le déplace automatiquement vers la Bibliothèque. Le nettoyage de l'indexeur est désormais limité aux lignes possédant un `fileId`, avec une protection supplémentaire testée contre la suppression d'une ligne de Ma liste.

La barre inférieure de cette branche contient Accueil, Bibliothèque et Ma liste. La recherche a été déplacée dans les barres supérieures de ces trois écrans ; elle s'ouvre comme un écran secondaire avec une flèche retour et sans barre inférieure. Les écrans Bibliothèque et Ma liste conservent chacun les sous-onglets Films et Séries. Sur une fiche, le bouton indique maintenant « Ajouter à ma liste », « Dans ma liste » ou « Dans la bibliothèque » et ne peut plus supprimer une ligne Drive. La déconnexion conserve la dernière Bibliothèque connue au lieu de transformer toutes ses lignes en éléments de Ma liste.
...
La bannière recherche d'abord, par date `modifiedTime`, un film ou une série de la Bibliothèque Drive dont les détails TMDB fournissent un `poster_path`. En l'absence de candidat Drive valable, elle choisit une sortie cinéma française avec le libellé « Au cinéma », puis un média disponible en streaming et affiche sa première plateforme française avec l'attribution JustWatch. Le contenu Drive porte aussi le libellé « Sur votre Drive ».

Sa présentation locale adopte désormais une grande affiche verticale `2:3` inspirée de Netflix. L'ancien arrière-plan horizontal flouté et la carte intérieure ont été supprimés. Un dégradé inférieur reçoit le libellé de disponibilité, le titre, le type, l'année, la note et un bouton « Voir la fiche ». La carte et le bouton conservent la navigation historique vers la fiche du média : aucun lancement direct ou ajout à Ma liste n'est simulé depuis la bannière.

Continuer la lecture ne conserve que les progressions strictement positives et au plus égales à 90 %, associées à une entrée Drive toujours indexée ou à un fichier hors ligne encore présent. Les médias de cette rangée sont exclus de Sur votre Drive. Les nouveaux historiques mémorisent le `fileId` exact et le booléen hors ligne du `PlaybackItem`, y compris pour l'épisode précis d'une série. La base historique `zplex_db.db` passe donc de la version 2 à la version 3 avec `MIGRATION_2_3`, qui ajoute les colonnes nullable `fileId` et non nullable `offline` sans supprimer l'historique existant. Les anciennes lignes sans ces informations continuent d'utiliser l'appartenance Drive ou la correspondance exacte d'un téléchargement hors ligne.
